...
//Depth sensor stuff starts here
...
double pressure_abs_mbar, pressure_baseline, pressure_abs_dbar;

//Depth sensor stuff ends here
...
//Stat up stuff
//This number will help ditermine if this is a startup run or a recording to SD run
int Sd_yes = 0;
...
//Sensor calibration constant
  ...
  File dataFile = SD.open("First_test.txt", FILE_WRITE);

  // if the file is available, write to it:
  if (dataFile) {
    dataFile.println("time,temp,pH,cond,DO,Depth");
    dataFile.close();
  }
...
void loop() {
 ...
 //helpa ditermine if this run will record to sd
 Sd_yes++;
 }
...
//Start of pressure readings
  ...
  pressure_abs_mbar = sensor.getPressure(ADC_4096);
  pressure_abs_dbar = (pressure_abs_mbar*0.01);
 ...
 void step4() { //print data to serial monitor and to SD card
  ...
  dataString += String(pressure_abs_dbar);
  Serial.println();
...
//Has to do with the number of runs
if (Sd_yes = 50) {
Sd_yes = 0;
delay(700000);
}
